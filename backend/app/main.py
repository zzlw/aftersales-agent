...
from app.rag.ingest import ingest_if_changed
...
async def lifespan(app: FastAPI):
    ...
    # 知识库内容指纹比对：首次部署或语料变更时自动重建索引，云端部署后无需手动 reindex
    stats = await ingest_if_changed(pool)
    if stats:
        print(f"[startup] knowledge changed, auto-ingested: {stats}")
